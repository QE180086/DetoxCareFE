...
export default function Profile() {
  ...
  const [userData, setUserData] = useState({
    ...
    vouchers: [
      { code: 'DETOX10', discount: '10%', expiry: '2025-12-31', type: 'percentage' },
      { code: 'FREESHIP', discount: 'Free Shipping', expiry: '2025-11-30', type: 'shipping' },
      { code: 'DETOX20', discount: '20%', expiry: '2025-10-15', type: 'percentage' },
      { code: 'SUMMER50', discount: '50.000đ', expiry: '2025-09-30', type: 'fixed' },
    ],
  ...
  useEffect(() => {
    if (profile) {
      setUserData((prev) => ({
        ...
        gender: profile.gender ? (profile.gender === 'MALE' ? 'Nam' : profile.gender === 'FEMALE' ? 'Nữ' : prev.gender) : prev.gender,
  ...
  const handleCancel = () => {
    setUserData({
      ...
      gender: profile?.gender ? (profile?.gender === 'MALE' ? 'Nam' : profile?.gender === 'FEMALE' ? 'Nữ' : 'Nam') : 'Nam',
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-blue-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-gray-600 text-lg">Quản lý thông tin cá nhân và theo dõi hành trình detox của bạn</p>
        </div>

        {loading && (
          <div className="text-center py-12">
            <div className="inline-block animate-spin rounded-full h-12 w-12 border-4 border-green-500 border-t-transparent"></div>
            <p className="text-green-600 font-medium mt-4">Đang tải thông tin...</p>
          </div>
        )}
            ...
            <div className="lg:col-span-1">
              ...
              <div className="bg-white rounded-2xl shadow-xl overflow-hidden mb-6">
                <div className="bg-gradient-to-r from-green-600 to-blue-600 p-6 text-white text-center">
                  ...
                  <p className="text-sm opacity-90 mt-1">Thành viên từ {userData.joined || 'N/A'}</p>
                  <div className="mt-3 px-3 py-1 bg-white bg-opacity-20 rounded-full inline-block">
                    <span className="text-sm font-medium">{userData.membershipLevel || 'N/A'}</span>
                  </div>
                </div>
                <div className="p-6 bg-gradient-to-r from-green-50 to-blue-50">
                  <div className="flex justify-center items-center gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-600">{userData.points !== null ? userData.points : 'N/A'}</div>
                      <div className="text-sm text-gray-600">Điểm tích lũy</div>
                    </div>
                    <div className="w-px h-12 bg-gray-300"></div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-blue-600">{userData.vouchers?.length || 0}</div>
              ...
              <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
                ...
                {activeTab === 'personal' && (
                  <div className="p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-2">
                        <label className="flex items-center gap-2 text-sm font-medium text-gray-600">
                          {userData.gender === 'Nam' ? <FaMale size={16} /> : <FaFemale size={16} />}
                          Giới tính
                        </label>
                        {isEditing ? (
                          <select
                    ...
                    <div className="mt-8 pt-8 border-t border-gray-200">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="px-4 py-3 bg-blue-50 rounded-lg">
                          ...
                          <div className="text-lg font-semibold text-blue-600">{userData.lastOrder || 'N/A'}</div>
                        </div>
                        <div className="px-4 py-3 bg-green-50 rounded-lg">
                          <div className="text-sm text-gray-600">Cấp độ thành viên</div>
                          <div className="text-lg font-semibold text-green-600">{userData.membershipLevel || 'N/A'}</div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {activeTab === 'rewards' && (
                  <div className="p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                      <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-xl p-6 text-white">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="text-2xl font-bold">{userData.points !== null ? userData.points : 'N/A'}</div>
                            <div className="text-sm opacity-90">Điểm hiện tại</div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl p-6 text-white">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="text-2xl font-bold">{userData.vouchers?.length || 0}</div>
                            <div className="text-sm opacity-90">Voucher khả dụng</div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-gradient-to-r from-purple-500 to-purple-600 rounded-xl p-6 text-white">
                        <div className="flex items-center gap-3">
                          <FaLeaf size={24} />
                          <div>
                            <div className="text-lg font-bold">{userData.membershipLevel || 'N/A'}</div>
                            <div className="text-sm opacity-90">Cấp thành viên</div>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Vouchers Grid */}
                    <div>
                      <h3 className="text-xl font-semibold text-gray-800 mb-6">Voucher của bạn</h3>
                      {userData.vouchers && userData.vouchers.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {userData.vouchers.map((voucher, index) => (
                            <div
                              key={index}
                              className={`${getVoucherColor(voucher.type)} rounded-2xl p-6 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1`}
                            >
                                ...
                                <div className="flex-1">
                                  <div className="flex items-center gap-3 mb-3">
                                    ...
                                    <div>
                                      ...
                                      <div className="text-sm opacity-90">Mã voucher</div>
                                    </div>
                                  </div>
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="text-sm opacity-90">Hết hạn: {voucher.expiry}</span>
                                    </div>
                                  </div>
                                </div>
                        ...
                        <div className="text-center py-12">
                          <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
                            <AiOutlineGift size={32} className="text-gray-400" />
                          </div>
                          <p className="text-gray-500 text-lg font-medium">Chưa có voucher nào</p>
                          <p className="text-gray-400 text-sm mt-2">Tích điểm để nhận voucher hấp dẫn!</p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="mt-12 bg-gradient-to-r from-green-50 to-blue-50 rounded-2xl p-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div className="flex items-center gap-3 p-3 bg-white rounded-lg">
                          ...
                          <span className="text-gray-700">Mỗi đơn hàng thành công</span>
                        </div>
                        <div className="flex items-center gap-3 p-3 bg-white rounded-lg">
                          <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
                            <span className="text-blue-600 font-bold">+5</span>
                          </div>
                          <span className="text-gray-700">Chia sẻ sản phẩm</span>
                        </div>
                        <div className="flex items-center gap-3 p-3 bg-white rounded-lg">
                          <div className="w-8 h-8 bg-purple-100 rounded-full flex items-center justify-center">
                            <span className="text-purple-600 font-bold">+20</span>
                          </div>
                          <span className="text-gray-700">Đánh giá sản phẩm</span>
                        </div>
                        <div className="flex items-center gap-3 p-3 bg-white rounded-lg">
                          <div className="w-8 h-8 bg-orange-100 rounded-full flex items-center justify-center">
                            <span className="text-orange-600 font-bold">+50</span>
                          </div>
                          <span className="text-gray-700">Giới thiệu bạn bè</span>
                        </div>
                      </div>
                    </div>
                  </div>
